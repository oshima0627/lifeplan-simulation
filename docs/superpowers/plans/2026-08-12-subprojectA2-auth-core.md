...
2. **登録済みかどうかを応答から判別できないようにする。** `signup` でメール重複だったときの
   文言は「このメールアドレスは登録できません」のような**存在を明かさないもの**にする。
   `login` の失敗も「メールアドレスまたはパスワードが違います」の一本にし、
   **ユーザーが存在しない場合と鍵が違う場合で文言もステータスも変えない**
3. **`logout` は DB の削除に失敗しても Cookie は必ず落とす。** ログアウトを失敗させない
4. `Secure` 属性は `new URL(request.url).protocol === "https:"` で判定する
   （ローカル開発の http で Cookie が落ちなくなるのを避ける）
